...
-- Create Instance dengan performa lebih baik
function Util.create(className, properties, parent)
    local obj = Instance.new(className)
    -- Set parent terakhir untuk optimasi performa rendering
    for prop, value in pairs(properties or {}) do
...
-- ── Systems.Notification (UPGRADED & LIGHTWEIGHT) ──────────────────────────────
...
-- Konfigurasi tampilan agar pas di Mobile & PC
...
function Notification.new(screenGui, theme)
    ...
    self._container = Util.create("Frame", {
        Name              = "GenUI_NotifContainer",
        AnchorPoint       = Vector2.new(1, 1),
        Position          = UDim2.new(1, -EDGE_MARGIN, 1, -EDGE_MARGIN),
        Size              = UDim2.new(0, CARD_WIDTH, 1, -100),
...
function Notification:push(options)
    ...
    -- Gunakan CanvasGroup agar animasi Fade sangat ringan (Optimasi Mobile)
    -- Dan otomatis memperbaiki rendering lengkungan di pojok
    local card = Util.create("CanvasGroup", {
        Name             = "Notif_" .. id,
        Size             = UDim2.new(1, 0, 0, 0), -- Mulai dari 0 untuk animasi
    ...
    Util.stroke(card, self._theme:get("Border"), 1.2)
    ...
    local contentLayout = Util.listLayout(inner, { Padding = UDim.new(0, 4) })
    ...
    Util.listLayout(topRow, { FillDirection = Enum.Horizontal, VerticalAlignment = Enum.Center, Padding = UDim.new(0, 6) })
    ...
    if options.Icon then
        ...
        Icons.apply(img, options.Icon, self._theme:get("Accent"))
    end
    ...
    -- Isi Pesan (AutomaticSize agar enteng)
    ...
    -- Animasi Masuk (Size + Fade)
    local targetHeight = options.Content and 65 or 45
    Tween.to(card, { Size = UDim2.new(1, 0, 0, targetHeight), GroupTransparency = 0 }, 0.3, Enum.EasingStyle.Quart)

    -- Auto-dismiss
    task.delay(options.Duration or 3, function()
        if card and card.Parent then
            local out = Tween.to(card, { GroupTransparency = 1, Size = UDim2.new(1, 0, 0, 0) }, 0.2)
